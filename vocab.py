...
class MotifVocab(object):
    ...
    def __init__(self, pair_list: List[Tuple[str, str]]):
        ...
        self.bond_type_conns_dict = bond_type_motifs_dict
         ###
        for motif_idx, motif_smiles in enumerate(self.motif_smiles_list):  
            if motif_idx >= len(self.vocab_conn_dict):  
                print(f"Error: motif_idx {motif_idx} is out of range. Max index: {len(self.vocab_conn_dict)-1}")  
                break  
        # Rest of the code  
...
class SubMotifVocab(object):
    ...
    def __init__(self, motif_vocab: MotifVocab, sublist: List[int]):
        ...
        ###
        for i, mid in enumerate(sublist):  
            if mid >= len(self.motif_vocab.vocab_conn_dict):  
                print(f"Error: mid {mid} is out of range. Max index: {len(self.motif_vocab.vocab_conn_dict)-1}")  
                break  
        # Rest of the code 
